    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        CoroutineScope(SupervisorJob() + Dispatchers.IO).launch {
            try {
                ...
                JobScheduler(application).rebook(marketRefresh, closeSweep, priceSeries)
                // Swept while anything is on, and cancelled only when everything is off - the same
                // shape as the daily check, and for the same reason.
                if (marketRefresh || closeSweep || priceSeries) {
